...
if (!class_exists('KcSeoMetaData')):
    ...
    class KcSeoMetaData
    {
        ...
        function __construct() {
            ...
            //custom category archive meta 

            $settings_schema = get_option('kcseo_wp_schema_settings'); 

            $supported_taxonomy = [];

            if ( isset($settings_schema['archive']) && $settings_schema['archive'] == '1') {
                $supported_taxonomy[] = 'category';
                $supported_taxonomy[] = 'post_tag';
            }

            if ( isset($settings_schema['product_archive']) && $settings_schema['product_archive'] == '1') {
                $supported_taxonomy[] = 'product_cat';
                $supported_taxonomy[] = 'product_tag';
            }

            foreach( $supported_taxonomy as $taxonomy ) {
                add_action( 'edit_' . $taxonomy, [$this, 'save_archive_KcSeo_schema_data'], 10, 1 );    
                add_action( $taxonomy .'_edit_form', [$this, 'meta_archive_box_wp_schema'], 10, 1 );  
            }
        ...
        function meta_box_wp_schema($post) {
            ...
            $_kcseo_ative_tab = get_post_meta($post_id, '_kcseo_ative_tab', true);
            ...
            $html .= "<p><span>Tip:</span> " . __("Once you save these structured data schema settings, validate this page url here:", "wp-seo-structured-data-schema-pro") . " <a href='https://developers.google.com/search/docs/advanced/structured-data'>https://developers.google.com/search/docs/advanced/structured-data</a></p>";
            $html .= "</div>";
            ...
            foreach ($schemaFields as $schemaID => $schema) {
                ...
                $metaData = get_post_meta($post_id, $tabId, true);
                ...
                $multipleMetaData = get_post_meta($post_id, $tabId . "_multiple", true);
                $multipleHtml = null;
        ...
        function meta_archive_box_wp_schema($term) {
            global $KcSeoWPSchema; 

            $term_id = $term->term_id;
            wp_nonce_field($KcSeoWPSchema->nonceText(), '_kcseo_nonce');
            $_kcseo_ative_tab = get_term_meta($term_id, '_kcseo_ative_tab', true);
            ...
            foreach ($schemaFields as $schemaID => $schema) {
                ...
                $htmlCont .= "<div id='{$tabId}' class='rt-tab-content{$activeClass}'>";
                $htmlCont .= "<div class='kc-top-toolbar'><span class='kc-auto-fill button button-primary'>" . __("Auto Fill", "wp-seo-structured-data-schema-pro") . "</span></div>";
                $metaData = get_term_meta($term_id, $tabId, true);
                $metaData = (is_array($metaData) ? $metaData : array());
                foreach ($schema['fields'] as $fieldId => $data) {
                    $data['fieldId'] = $fieldId;
                    $data['id'] = $tabId . "_" . $fieldId;
                ...
                $multipleMetaData = get_term_meta($term_id, $tabId . "_multiple", true);
                $multipleHtml = null;
                if (!empty($multipleMetaData) && is_array($multipleMetaData)) {
                    foreach ($multipleMetaData as $mid => $mMetaDatum) {
                        $multipleHtml .= "<div class='multiple-schema-item'>";
        ...
        function save_archive_KcSeo_schema_data($term_id) { 
            ...
            foreach ($schemaFields as $schemaID => $schema) {
                ...
                if ($multiple_data && is_array($multiple_data)) {
                    ...
                    delete_term_meta($term_id, $schemaMetaId . "_multiple");
                }
            }
            ...
            if (count($meta) > 0) {
                foreach ($meta as $mKey => $mValue) {
                    update_term_meta($term_id, $mKey, $mValue);
                }
            }
        }
