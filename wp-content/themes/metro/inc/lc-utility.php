...
class Utility { 
    ...
    function register_notice() {
        ...
        if ( isset( rtlc_is_valid()['domain_match'] ) && !rtlc_is_valid()['domain_match'] ) { 
            $message = sprintf(
                wp_kses( 
                    __( '<b>Your license key doesn\'t match your current domain. Please activate the license again for your current domain. <a href="%s">Activate License</a></b>', 'metro' ),
                    [
                        'a' => [ 'href' => [], 'target' => [] ],
                        'b' => []
                    ]
            ...
            $message = sprintf(
                wp_kses( 
                    __( '<b>Please activate your theme using Envato <a href="%s" target="_blank">purchase code</a>, to get full theme functionality and customer support. <a href="%s">Activate License</a></b>', 'metro' ),
                    [
                        'a' => [ 'href' => [], 'target' => [] ],
                        'b' => []
                    ]
    ...
    function script() { 
                ...
                if ( purchase_code ) {
                    jQuery.ajax({
                        ...
                        success: function( resp ) {   
                            if ( resp === 555 ) {
                                alert('<?php esc_html_e('Purchase code already activated for one domain!!!', 'metro'); ?>'); 
                            } else {
                                if ( resp ) {
                                    jQuery('.rtlc-status-btn').html('<?php esc_html_e('Activated', 'metro'); ?>');
                                    jQuery('.rtlc-status-btn').removeClass('rtlc-unverified').addClass('rtlc-verified');
                                } else {
                                    alert('<?php esc_html_e('Sorry!!! Purchase code does not match', 'metro'); ?>');
                                }
                            } 
                    ...
                    alert('<?php esc_html_e('Purchase code is required!!!', 'metro'); ?>');
                } 
            });
        </script> 
        <?php
        ...
        if ( wp_script_is( 'jquery', 'done' ) && !rtlc_is_valid()['success'] ) { 
            if ( isset( $_GET['page'] ) && $_GET['page'] == 'fw-backups-demo-content' ) { ?>
                <script type="text/javascript"> 
                    jQuery("#fw-ext-backups-demo-list .theme-actions a").on("click", function(e) { 
                        e.preventDefault();
                        alert('<?php esc_html_e('Please activate your theme using Envato purchase code, to install demo data.', 'metro'); ?>');
                        return false;
                    });
                </script> 
            <?php
            ...
            if ( isset( $_GET['page'] ) && $_GET['page'] == 'metro-install-plugins' ) { ?>
                <script type="text/javascript">  
                    jQuery(".row-actions a").on("click", function(e) {  
                        if ( jQuery(this).closest('td').next().has('span').length > 0 ) { //find pre packaged
                            e.preventDefault();
                            alert('<?php esc_html_e('Please activate your theme using Envato purchase code, to use this plugin.', 'metro'); ?>');
                            return false;
                        } 
                    });
...
if ( !function_exists('rtlc_is_valid') ) {
    function rtlc_is_valid() {
        ...
        $metro = $theme_info->get('Name');  

        //replace space
        $metro = strtolower(trim(preg_replace('/[^A-Za-z0-9-]+/', '-', $metro))); 
        $get_option = get_option('rt_licenses');

        //this first line is for checking old codebase
        if ( isset( $get_option[$metro.'_license_key'] ) ) {
            return [
                'success' => true,
                'domain_match' => true
            ];
        } else if ( isset( $get_option[$metro.'_license'] ) && isset( $get_option[$metro.'_license']['key'] ) ) {
            $domian_name = esc_url( home_url() ); 
            $domain = $get_option[$metro.'_license']['domain']; 
